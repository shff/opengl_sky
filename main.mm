...
int main(int argc, char **argv)
{
  @autoreleasepool
  {
    ...
    makeEntity(&s, skyVertShader, skyFragShader, 0, NULL, NULL, 4, 0, 0, 0, 0,
               0);
    makeEntity(&s, postVertShader, postFragShader, 0, NULL, NULL, 4, 0, 1, 0,
               800, 600);
    ...
    while (running)
    {
      ...
      if ([keysDown containsObject:@"a"])
        mx -= 1.0f;
      else if ([keysDown containsObject:@"d"])
        mx += 1.0f;
      if ([keysDown containsObject:@"w"])
        my -= 1.0f;
      else if ([keysDown containsObject:@"s"])
        my += 1.0f;

      s.state.r -= (float)(mx) * 0.01f;
      s.state.r2 -= (float)(my) * 0.01f;
      ...
      renderScene(s, 800, 600, ticks * 0.001f);
      glFlush();
